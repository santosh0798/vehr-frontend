...
const ViewBonus = () => {
    ...
    React.useEffect(() => {
        ...
        async function getAtt() {
            const x = await axios.get(`http://3.86.184.176:4000/api/v1/employee/bonus/mylist/${date.getFullYear()}?page=${page}`, {
                withCredentials: true
            });
            setAttend(x.data);

            const y = await axios.get(`http://3.86.184.176:4000/api/v1/employee/bonus/mylist/${date.getFullYear()}?page=${0}&limit=${999999999}`, {
                withCredentials: true
            });
            setPdfAttend(y.data);
        }
