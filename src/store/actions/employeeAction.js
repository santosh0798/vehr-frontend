...
export const addEmployee = (userData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(' https://3.86.184.176/api/v1/employees/new', y, config);
...
export const myEmployee = (page, limit) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`https://3.86.184.176/api/v1/employees/mylist?page=${page}${limit && '&limit=' + limit}`, {
            withCredentials: true
        });
        console.log(data);
        dispatch({
